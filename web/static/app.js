...
async function previewFile(taskId, path, renderToken = 0) {
  ...
  try {
    ...
    const diffText = await fetchFileDiff(taskId, path);
    if (state.activeTaskId !== taskId || previewToken !== state.previewToken || (renderToken && renderToken !== state.fileViewerToken)) return;
    if (stringsHaveContent(diffText)) {
      setFilePreviewHTML(targetKey, 'file-preview diff-preview', renderUnifiedDiff(diffText));
      return;
    }
...
async function fetchFileDiff(taskId, path) {
  try {
    return await fetchText(`${taskAPIPath(taskId, '/files')}?path=${encodeURIComponent(path)}&diff=1`);
  } catch {
    return '';
  }
}

function stringsHaveContent(value) {
...
function renderUnifiedDiff(diffText) {
  ...
  const rows = visible.map(line => {
    ...
    return `<tr class="diff-row ${parsed.kind}"><td class="diff-line-no old">${parsed.oldNo}</td><td class="diff-line-no new">${parsed.newNo}</td><td class="diff-marker">${escapeHTML(parsed.marker)}</td><td class="diff-code"><code>${escapeHTML(parsed.text) || '&nbsp;'}</code></td></tr>`;
  }).join('');
  const truncated = lines.length > maxLines ? `<div class="preview-note">${escapeHTML(t('previewTruncated').replace('%d', String(diffText || '').length))}</div>` : '';
  return `${truncated}<div class="diff-wrap"><div class="diff-title">${escapeHTML(t('fileDiff'))}</div><table class="diff-table"><tbody>${rows}</tbody></table></div>`;
}

function parseDiffLine(line, oldLine, newLine) {
  const hunk = /^@@ -(\d+)(?:,\d+)? \+(\d+)(?:,\d+)? @@/.exec(line);
  if (hunk) {
    return { kind: 'hunk', marker: '', text: line, oldNo: '', newNo: '', nextOldLine: Number(hunk[1]), nextNewLine: Number(hunk[2]) };
  }
  if (line.startsWith('diff --git') || line.startsWith('index ') || line.startsWith('--- ') || line.startsWith('+++ ')) {
    return { kind: 'meta', marker: '', text: line, oldNo: '', newNo: '', nextOldLine: oldLine, nextNewLine: newLine };
  }
  if (line.startsWith('+')) {
    const no = newLine == null ? '' : String(newLine);
    return { kind: 'add', marker: '+', text: line.slice(1), oldNo: '', newNo: no, nextOldLine: oldLine, nextNewLine: newLine == null ? newLine : newLine + 1 };
  }
  if (line.startsWith('-')) {
    const no = oldLine == null ? '' : String(oldLine);
    return { kind: 'del', marker: '-', text: line.slice(1), oldNo: no, newNo: '', nextOldLine: oldLine == null ? oldLine : oldLine + 1, nextNewLine: newLine };
  }
  const oldNo = oldLine == null ? '' : String(oldLine);
  const newNo = newLine == null ? '' : String(newLine);
  return { kind: 'ctx', marker: line ? ' ' : '', text: line.startsWith(' ') ? line.slice(1) : line, oldNo, newNo, nextOldLine: oldLine == null ? oldLine : oldLine + 1, nextNewLine: newLine == null ? newLine : newLine + 1 };
}
