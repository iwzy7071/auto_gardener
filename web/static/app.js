...
$('sendMessageBtn').onclick = async () => {
  ...
  const modeSelect = $('messageModeSelect');
  const content = input.value.trim();
  if (!state.activeTaskId || !content) return;
  const mode = modeSelect?.value === 'ask' ? 'ask' : 'run';
  const taskId = state.activeTaskId;
  const originalTask = state.tasks.find(t => t.id === taskId);
  ...
  if (originalTask) {
    ...
    if (mode !== 'ask') {
      ...
      optimistic.gardenerStatus = 'Running';
    }
    upsertTask(optimistic);
    renderTask(optimistic, { skipFileViewer: true });
  }
  try {
    const data = await api(taskAPIPath(taskId, '/messages'), { method:'POST', body: JSON.stringify({ content, mode }) });
    upsertTask(data.task);
    if (state.activeTaskId === taskId) renderTask(data.task, { skipFileViewer: true });
  } catch(err){
    input.value = content;
