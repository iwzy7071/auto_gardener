...
async function renderAcceptance(task) {
  const panel = $('acceptancePanel');
  if (!panel || !task?.id) return;
  const cached = state.acceptance[task.id];
  if (cached) paintAcceptance(panel, cached);
  const now = Date.now();
  const freshFor = task.status === 'Running' ? 15000 : 60000;
  if (state.acceptancePending[task.id] || (cached && now - (state.acceptanceFetchedAt[task.id] || 0) < freshFor)) return;
  state.acceptancePending[task.id] = true;
  try {
    const data = await api(taskAPIPath(task.id, '/acceptance'));
    state.acceptance[task.id] = data.acceptance;
    state.acceptanceFetchedAt[task.id] = Date.now();
    if (state.activeTaskId === task.id) paintAcceptance(panel, data.acceptance);
  } catch (err) {
    if (!cached) panel.classList.add('hidden');
  } finally {
    state.acceptancePending[task.id] = false;
  }
}

function paintAcceptance(panel, report) {
  if (!report) { panel.classList.add('hidden'); return; }
  const status = String(report.status || 'pending');
  const label = acceptanceStatusText(status);
  ...
  panel.className = `acceptance-panel ${status}`;
  panel.innerHTML = `
    <div class="acceptance-head"><div><strong>${t('acceptance')}</strong><p>${escapeHTML(report.summary || '')}</p></div><div class="acceptance-score"><span>${t('qualityScore')}</span><b>${Number(report.score || 0)}</b><em>${escapeHTML(label)}</em></div></div>
    <div class="acceptance-checklist"><span>${t('acceptanceChecklist')}</span>${checklist.map(item => `<div class="acceptance-item ${escapeHTML(item.status || 'warning')}"><b>${acceptanceIcon(item.status)}</b><div><strong>${escapeHTML(item.label || '')}</strong>${item.evidence ? `<p>${escapeHTML(item.evidence)}</p>` : ''}</div></div>`).join('')}</div>
    ${risks.length ? `<div class="acceptance-notes"><span>${t('risks')}</span>${risks.map(line => `<p>${escapeHTML(line)}</p>`).join('')}</div>` : ''}
    ${nextSteps.length ? `<div class="acceptance-notes"><span>${t('nextSteps')}</span>${nextSteps.map(line => `<p>${escapeHTML(line)}</p>`).join('')}</div>` : ''}
  `;
}

function acceptanceStatusText(status) {
  if (status === 'passed') return t('acceptancePassed');
  if (status === 'partial') return t('acceptancePartial');
  if (status === 'blocked') return t('acceptanceBlocked');
  return t('acceptancePending');
}

function acceptanceIcon(status) {
