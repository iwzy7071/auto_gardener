...
- Watchdog background self-checks for long-running tasks that stop producing output; users are prompted only when Gardener needs confirmation.
...
| `AUTO_GARDENER_MINIMAX_TOKEN` / `AUTO_GARDENER_KIMI_TOKEN` | Optional bundled provider keys used when local settings do not already contain MiniMax/Kimi keys. |
...
Then edit the `.local` file. It is intentionally ignored by git. For nginx/relay installs, set `GARDENER_RELAY_MINIMAX_TOKEN_FILE` and/or `GARDENER_RELAY_KIMI_TOKEN_FILE` on the relay server to seed provider keys into the one-time provision JSON; installers write them into the local owner-only `forest_data/settings.json` so users do not need to paste keys in the UI.
...
packaging/linux/            Linux/Ubuntu install/start/update scripts
...
make linux-build   # cross-compile Linux server
...
- Watchdog 会在长任务长期无输出时先让 Gardener 后台自查；只有需要用户确认时才提示用户。
...
| `AUTO_GARDENER_MINIMAX_TOKEN` / `AUTO_GARDENER_KIMI_TOKEN` | 可选的内置 provider key；本地设置没有 MiniMax/Kimi key 时自动使用。 |
...
然后编辑 `.local` 文件。该文件会被 git 忽略。通过 nginx/relay 安装时，可在中转服务器设置 `GARDENER_RELAY_MINIMAX_TOKEN_FILE` 和/或 `GARDENER_RELAY_KIMI_TOKEN_FILE`，安装 provision 会把 provider key 写入用户本机 owner-only 的 `forest_data/settings.json`，用户无需再在界面粘贴 SK。
...
packaging/linux/            Linux/Ubuntu 安装/启动/升级脚本
...
make linux-build   # 交叉编译 Linux 服务端
make check         # 全部核心检查
```
