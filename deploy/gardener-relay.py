...
def mac_install_command(setup_key):
    return f'curl -fsSL --connect-timeout 20 --max-time 120 {MAC_INSTALL_SCRIPT_URL} -o install-gardener-macos.sh && bash install-gardener-macos.sh --relay-base-url {RELAY_PUBLIC_BASE_URL} --setup-key {setup_key}'
