...
const maxFruitReportFieldRunes = 1000
...
`, fruitReportField(tr.ID), fruitReportField(task.ID), fruitReportField(task.Title), fruitReportField(tr.Name), start.Format(time.RFC3339), end.Format(time.RFC3339), fruitReportField(task.WorkspacePath), fruitReportField(taskWorkDir(task)), fruitReportField(strings.Join(tr.Scope, ", ")), tr.IsValidation, fruitReportField(tr.Objective), strings.TrimSpace(output), fruitReportField(errText), codex.Truncate(output, 1200))
	return path, os.WriteFile(path, []byte(body), 0644)
}

func fruitReportField(s string) string {
	return codex.Truncate(s, maxFruitReportFieldRunes)
}
