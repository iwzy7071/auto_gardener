...
func TestWriteMockDeliverableRejectsSymlink(t *testing.T) {
	...
	link := filepath.Join(dir, "mock-deliverable.md")
	if err := os.Symlink(target, link); err != nil {
		t.Fatal(err)
	}

	err := writeMockDeliverable(link)
	if err == nil || !strings.Contains(err.Error(), "symlink") {
		t.Fatalf("expected symlink mock deliverable to be rejected, got %v", err)
	}
